...
export default function ModalEditSection({ open, setOpen, section, setSection }) {
    const { myPages, setMyPages, setOpenLoader } = useContext(ShopContext)
    ...
    const requestEditSection = async (data) => {
        try {
            setOpenLoader(true);
            const { response, status } = await actionSetions.editSection(data);
            if (status === 200) {
                setSection(response);
                resetValues();
                setOpenLoader(false);
                enqueueSnackbar(`¡Sección editada exitosamente! 🎉`, {
                    variant: 'success',
                    anchorOrigin: {
                        vertical: 'top',
                        horizontal: 'right',
